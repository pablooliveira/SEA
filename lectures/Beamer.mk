...
$(1)-print.tex: $(MAKEFILE_LIST) $(EXTRAPRINT)
	...
	echo '\footskip 2.7cm' >> $$@~
	echo '\begin{document}' >> $$@~
	echo '\includepdf[pages=-,offset=0in 0in,nup=2x3,frame=false,noautoscale=false,delta=0.2cm 0.3cm,scale=0.95,pagecommand={\thispagestyle{empty}}]{$(1).pdf}' >> $$@~
	for file in $(EXTRAPRINT); do \
		echo "\includepdf[pages=-]{$$$$file}" >> $$@~; \
	done
	echo '\end{document}' >> $$@~
	mv -f $$@~ $$@
...
clean:
	...
	rm -f *~ *.dvi *.vrb *.nav *.snm *.pyg texput.* *.synctex.gz
	rm -rf _minted-*/ auto/
