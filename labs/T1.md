...
utiliser le serveur gitlab [tahiti.prism.uvsq.fr](http://tahiti.prism.uvsq.fr).

(@) Normalement vous avez reçu un mail sur votre adresse `@isty.uvsq.fr` vous invitant à créer un compte sur [tahiti.prism.uvsq.fr](http://tahiti.prism.uvsq.fr). Attention il est impératif d'utiliser cette adresse mail et pas une adresse personnelle lors de la création du compte.

(@) Lorsque vous vous connectez sur le site, vous pourrez voir vos projets. Un projet `sea18/prenom-nom` a déjà été crée pour vous. 
...
* Utiliser une machine virtuelle distante comme [codeanywhere](https://codeanywhere.com) ou Cloud 9.

### Initialization du dépôt

(@) Pour pouvoir commencer à travailler sur votre dépôt git, il va falloir le cloner. Pour
 cela allez sur [tahiti.prism.uvsq.fr](http://tahiti.prism.uvsq.fr) et récupérez
...
Comme vous avez cloné votre dépôt depuis tahiti.prism.uvsq.fr; ce serveur est déja configuré
...
```bash
    ...
    To http://pablo-oliveira@tahiti.prism.uvsq.fr/sea18/pablo-oliveira.git
